...
function buildConfig(opts) {
  ...
  const hostnameValue = ' https://qa-mickey.planx-pla.net/';
  // const hostnameParts = hostnameValue.split('.');
  // const hLen = hostnameParts.length;
  /*   const hostnameNoSubdomain =
    hLen > 2 ? hostnameParts.splice(hLen - 2).join('.') : hostnameValue; */
  // const hostnameNoSubdomain = ' https://qa-mickey.planx-pla.net/';
  ...
  const defaults = {
    ...
    /* hostnameOnly:
      typeof window !== 'undefined' ? hostnameNoSubdomain : 'localhost', */
    hostnameOnly: ' https://qa-mickey.planx-pla.net/',
    hostname:
      /*  typeof window !== 'undefined'
        ? `${window.location.protocol}//${window.location.hostname}/`
        :  */ 'https://qa-mickey.planx-pla.net/',
    // hostnameWithSubdomain: hostnameValue,
    hostnameWithSubdomain: 'https://qa-mickey.planx-pla.net/',
  ...
  //
  const ensureTrailingSlashBasename = `${defaults.basename}${
    defaults.basename.endsWith('/') ? '' : '/'
  }`;
  if (
    typeof window !== 'undefined' &&
    typeof window?.location !== 'undefined' &&
    window?.location.pathname.indexOf(
      `${ensureTrailingSlashBasename}dev.html`,
    ) === 0
  ) {
  ...
  const cohortMiddlewarePath =
    typeof cohortMiddlewareURL === 'undefined'
      ? `${hostname}cohort-middleware/`
      : ensureTrailingSlash(cohortMiddlewareURL);
  const gwasWorkflowPath =
    typeof gwasWorkflowURL === 'undefined'
      ? `${hostname}ga4gh/wes/v2/`
      : ensureTrailingSlash(gwasWorkflowURL);
  ...
  const requestorPath =
    typeof requestorURL === 'undefined'
      ? `${hostname}requestor/`
      : ensureTrailingSlash(requestorURL);
  ...
  if (config.ddSampleRate) {
    if (Number.isNaN(config.ddSampleRate)) {
      ...
      console.warn(
        'Datadog sample rate value in Portal config is not a number, ignoring',
      );
    } else {
  ...
  if (!grafanaFaroConfig.grafanaFaroSampleRate) {
    ...
    // eslint-disable-next-line no-console
    console.warn(
      'Grafana Faro sample rate value in Portal config is not a number, ignoring',
    );
    grafanaFaroConfig.grafanaFaroSampleRate = 1;
  }
  ...
  if (config.studyViewerConfig) {
    ...
    studyViewerConfig.forEach((cfg, i) => {
      if (cfg.openMode && !validOpenOptions.includes(cfg.openMode)) {
  ...
  if (components.index.homepageChartNodesExcludeFiles) {
    homepageChartNodesExcludeFiles =
      components.index.homepageChartNodesExcludeFiles;
  }
  ...
  if (
    config.featureFlags &&
    config.featureFlags.explorerHideEmptyFilterSection
  ) {
  ...
  if (
    config.featureFlags &&
    config.featureFlags.forceSingleLoginDropdownOptions
  ) {
  ...
  if (covid19DashboardConfig) {
    covid19DashboardConfig.dataUrl = ensureTrailingSlash(
      covid19DashboardConfig.dataUrl || '',
    );
  }
  ...
  if (!studyRegistrationConfig.studyRegistrationUIDField) {
    studyRegistrationConfig.studyRegistrationUIDField =
      discoveryConfig?.minimalFieldMapping?.uid;
  }
  ...
  const workspaceRegistrationConfig =
    registrationConfigs && registrationConfigs.features
      ? registrationConfigs.features.workspaceRegistrationConfig
      : null;
  const zendeskConfig = registrationConfigs
    ? registrationConfigs.zendeskConfig
    : null;
        ...
        switch (at) {
          case 'ndhHIV':
            analysisApps.ndhHIV = {
              ...
              description:
                'Classify stored clinical data based on controller status.',
  ...
  const commonsWideAltText = {
    ...
    'https://ctds.uchicago.edu/gen3':
      'Gen3 Data Commons - information and resources',
    'https://ctds.uchicago.edu/':
      'Center for Translational Data Science at the University of Chicago - information and resources',
  };
